...
def main():
    ...
    st.title("<� OASIS Form Automation Prototype")
    ...
    # Main content
    tab1, tab2, tab3 = st.tabs(["=� Generate Form", "=� View Documents", "9 About"])
...
def generate_form(patient_id, assessment_date, mode, transcript_file=None, history_file=None):
    ...
    with st.spinner("= Processing patient data..."):
        try:
            ...
            # Primary Diagnosis
            with st.expander("<� Primary Diagnosis"):
            ...
            # Cognitive Status
            with st.expander(">� Cognitive Status"):
            ...
            # Living Situation
            with st.expander("<� Living Situation"):
            ...
            st.download_button(
                label="=� Download OASIS Form (JSON)",
                data=json_data,
...
def display_documents(patient_id, mode):
    ...
    if mode == "Use Sample Data":
        ...
        if transcript_path.exists():
            with st.expander("=� Patient Visit Transcript", expanded=False):
